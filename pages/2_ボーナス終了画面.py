...
st.subheader("ボーナス終了画面")
st.caption("・ボーナス終了画面で示唆を確認")
st.caption("・AT当選でも表示されるため飛ばさないよう注意")
st.caption("・AT非当選時はPUSHボタンを押すとセリフで天井示唆あるため要確認")
...
#セレクトボックスを作成
st.session_state.pic_select = st.selectbox("終了画面", columns)

#選択肢に応じて画像を参考として表示させる
if st.session_state.pic_select == columns[0]:
    im = Image.open("./image/bonus_pic_shinji.jpg")
    st.image(im)

elif st.session_state.pic_select == columns[1]:
    im = Image.open("./image/bonus_pic_rei.jpg")
    st.image(im)

elif st.session_state.pic_select == columns[2]:
    im = Image.open("./image/bonus_pic_mari.jpg")
    st.image(im)

elif st.session_state.pic_select == columns[3]:
    im = Image.open("./image/bonus_pic_asuka.jpg")
    st.image(im)

elif st.session_state.pic_select == columns[4]:
    im = Image.open("./image/bonus_pic_rei_background.jpg")
    st.image(im)

elif st.session_state.pic_select == columns[5]:
    im = Image.open("./image/bonus_pic_godzieva.jpg")
    st.image(im)
...
##### 各画面の説明を追加する
df_result.loc["備考"] = ["デフォルト","偶数示唆","高設定示唆(弱)","高設定示唆(高)","設定2以上","設定4以上"]
